...
void main() {
  ...
  String vigenereDecrypt = Cryptography.vigenereCipher(vigenere, key, decrypt: true);
  ...
  try {
    ...
    print('Public Key (n, e): (${keys['publicKey']['n']}, ${keys['publicKey']['e']})');
    print('Private Key (n, d): (${keys['privateKey']['n']}, ${keys['privateKey']['d']})');
  ...
  for (String text in testTexts) {
    ...
    print('Text: "${text.substring(0, math.min(20, text.length))}"${text.length > 20 ? '...' : ''}');
    print('  Chi-squared after Vigenère: ${chi.toStringAsFixed(4)}');
  }
  
  print('\n🔬 Cryptography research complete!');
  print('This demonstrates basic cryptographic concepts for educational purposes.');
}

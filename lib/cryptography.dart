...
class Cryptography {
  ...
  /// Vigenère cipher
  static String vigenereCipher(String text, String key, {bool decrypt = false}) {
    String result = '';
    int keyIndex = 0;
    ...
    for (int i = 0; i < text.length; i++) {
      ...
      if (char.codeUnitAt(0) >= 65 && char.codeUnitAt(0) <= 90) {
        ...
        result += String.fromCharCode(((char.codeUnitAt(0) - 65 + shift + 26) % 26) + 65);
        keyIndex++;
      } else if (char.codeUnitAt(0) >= 97 && char.codeUnitAt(0) <= 122) {
        // Lowercase
        int shift = key[keyIndex % key.length].toLowerCase().codeUnitAt(0) - 97;
        if (decrypt) shift = -shift;
        result += String.fromCharCode(((char.codeUnitAt(0) - 97 + shift + 26) % 26) + 97);
        keyIndex++;
      } else {
        result += char;
      }
  ...
  static double chiSquaredTest(String text) {
    const expectedFrequencies = {
      'a': 0.0817, 'b': 0.0150, 'c': 0.0278, 'd': 0.0425, 'e': 0.1202,
